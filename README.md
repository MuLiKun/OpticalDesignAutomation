...
- 支持普通标准模板模式：GUI/CLI 可直接选择产品类型（RX/TX）、模板、公差等级、MC 次数、保存数量、补偿方式与是否保存 WC/BC；TX 评价函数不含 MTF，且固定不做后焦补偿 / TSC 优化（GUI 补偿器下拉在 TX 时置“无”并禁用）；默认自动使用主波长、自动识别公差结束面，并启用模板级视场映射（`标准分析` 目标 `0,0.5,0.9,-0.9`，`完整视场分析` 目标 `0,-0.25,0.25,-0.5,0.5,-0.7,0.7,-0.9,0.9,-1,1`）；运行期临时标准配置使用唯一文件名，避免覆盖用户导出的同名配置；标准模板公差向导仅保留面倾斜/元件偏心/元件倾斜（不含面偏心），并自动追加中心指向偏移（`POINTING_DY_F0_mm`/`POINTING_DX_F0_mm`）、焦距偏移百分比（`EFL_DELTA_PCT`）、视场角（`FOV_Y_deg`，RX 取物面 / TX 取像面）与多视场指向角偏差（`POINTING_ANGLE_F*_deg`，`RAID+CONS+DIFF` 正负偏差；标准分析取 `F0/F0.5/F0.9/F-0.9`，完整分析取正视场 `F0/F0.25/F0.5/F0.7/F0.9/F1`）评价项
...
- 读取 ZTD，把自定义脚本总值、REPORT 分项和 TDE 中的 `COMP` 补偿器项独立成列做统计，并导出统计 Excel；统计行含名义值（跑蒙卡前直接从评价函数读取，随 `report_meta` 落盘）与 Cpk1.33 上下限双边输出，按方向标黄用户关注侧
- 支持独立分析已有 ZTD；REPORT 表头优先从同名 TSC 的 `REPORT "标签" 行号` 反推，COMP 列优先用同目录 `run_config.json` 中的 TDE 顺序定位
- GUI 按分析模式自动收敛控件：高级 Excel 模式才显示 Excel 配置行与「检查配置」；提供「预览视场映射」按钮（Standalone 在临时工作副本模拟补齐缺失视场，用同一张表展示最终视场号、X/Y、归一化值与来源状态，不运行蒙卡、不改原始 zmx），以及「打开结果目录」按钮一键打开最近结果文件夹
...
普通标准模板模式（不需要手写 Excel，当前为后台/CLI/GUI 最小可用版；模板内容集中在 `tolerance_analysis/toltool/standard_templates.py`，可后续替换为正式标准）。当前行为：支持 RX/TX 产品类型（TX 评价函数不含 MTF，固定不做后焦补偿 / TSC 优化），模板为 `标准分析` / `完整视场分析`；主波长自动识别、结束面自动取像面前一面、优化补偿未填后焦补偿面时自动取像面前一面、标准模板视场映射默认开启并使用模板级目标视场，运行期临时配置生成 `<镜头名>_标准模板配置_<8位uuid>.xlsx`，GUI 可勾选保存 WC/BC：
...
- **2026-07-03** TX 差异化与 FOV/指向角评价：TX 标准模板改为独立模板（评价函数不含 MTF），并固定不做后焦补偿 / TSC 优化（`补偿器模式=无`、`后焦补偿面` 留空）；GUI 在「普通标准模板 + TX」下把补偿器下拉框置“无”并禁用；标准模板新增视场角 `FOV_Y_deg`（RX 取物面、TX 取像面，`REAC+ACOS+SUMM`）与多视场指向角偏差 `POINTING_ANGLE_F*_deg`（`RAID+CONS+DIFF` 正负偏差，标准分析取 `F0/F0.5/F0.9/F-0.9`、完整分析取正视场 `F0/F0.25/F0.5/F0.7/F0.9/F1`）评价项。
- **2026-07-02** 标准模板评价项与 ZTD 名义值：标准模板追加中心指向偏移 `POINTING_DY_F0_mm`/`POINTING_DX_F0_mm` 与焦距偏移百分比 `EFL_DELTA_PCT`（`EFL_DELTA_PCT` 的 `PROB` 因子写入第三参数 `100`）；标准模板公差向导移除面偏心，仅保留面倾斜/元件偏心/元件倾斜；ZTD 统计 Excel 新增“名义值”行，名义值改为跑蒙卡前直接从评价函数读取并随 `report_meta` 落盘 `run_config.json`。已知问题：真实环境下名义值仍读为空，已加告警日志与编辑器 Value 列读取 + 递归自算兜底，待实测确认。
