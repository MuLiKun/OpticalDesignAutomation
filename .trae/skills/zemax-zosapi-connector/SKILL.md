...
## 优先：直接读取 .zmx 文本（无需连接 Zemax）

**`.zmx` 是纯文本（ASCII）文件，可直接用文本工具读取解析，不必启动 Zemax、不必 ZOS-API 连接。** 当用户只是想**查看/提取镜头静态参数**（面型、曲率、厚度、材料、口径、视场、波长、光阑、系统设置等）时，直接 `Read` 文件即可，比连接 Zemax 快得多，也不消耗交互扩展等待状态。

何时直接读文本 vs 何时必须连 Zemax：

- **直接读 `.zmx` 文本即可**：读系统参数（`ENPD` 入瞳、`UNIT` 单位、`FTYP`/`XFLN`/`YFLN` 视场、`WAVM`/`PWAV` 波长、`GCAT` 玻璃库）、逐面数据（`SURF n` 块下 `TYPE`/`CURV`/`DISZ`/`DIAM`/`GLAS`/`CONI`/`PARM` 等）、注释与配置行。适合快速盘点、对比多个文件、离线解析、写脚本批量提取。
- **必须连 Zemax（ZOS-API）**：任何需要**计算/追迹**的结果——点列图、MTF、光程、像差、优化、公差分析、非序列探测器数据、求解后的实际值等。文本里只有输入定义，没有计算结果。

`.zmx` 关键行速查（按空格分列，值随文件而变）：

| 行标签 | 含义 |
|---|---|
| `VERS` | 文件格式版本 |
| `MODE` | `SEQ`(序列) / `NSC`(非序列) |
| `UNIT` | 单位系统（长度/角度/…），首字段常为 `MM` |
| `ENPD` | 入瞳直径 |
| `GCAT` | 使用的玻璃库 |
| `FTYP` | 视场类型（角度/物高/近轴像高/实际像高）与视场/波长数 |
| `XFLN`/`YFLN` | 各视场的 X/Y 值 |
| `FWGN` | 视场权重 |
| `WAVM i λ w` | 第 i 个波长（μm）及权重 |
| `PWAV` | 主波长序号 |
| `SURF n` | 第 n 面数据块起始（面 0 为物面） |
| `STOP` | 该面为光阑面 |
| `TYPE` | 面型（`STANDARD`/`EVENASPH`/`TOROIDAL`/`COORDBRK`…） |
| `CURV` | 曲率（1/半径，0 表示平面） |
| `DISZ` | 到下一面的厚度（`INFINITY` 表示无穷） |
| `DIAM` | 半口径 |
| `GLAS` | 材料名 |
| `CONI` | 圆锥系数 |
| `PARM i v` | 第 i 个面参数（非球面系数等） |

注意：`.zmx` 为 ASCII 文本；`.zos` 为较新的 XML/二进制封装格式，**不保证是纯文本**，若要读 `.zos` 优先仍走 ZOS-API 或先在 Zemax 里另存为 `.zmx`。半口径与曲率的正负号、坐标断点参数含义等细节，不确定时查 `zemax-manual-kb` 或用户手册。
