...
1. **定义公差**：序列模式用 `$sys.TDE.SEQToleranceWizard`（详见下方「TDE 向导实测」），设量级、关掉不需要的项、`OK()`。需精细覆盖则再直接写 TDE 行；补偿器 COMP 最后追加。
2. **运行工具**：`$tol=$sys.Tools.OpenTolerancing()`，配置后 `RunAndWaitForCompletion()`（阻塞）或 `Run()`+轮询（非阻塞），最后 `Close()`。

### TDE 向导实测（SEQToleranceWizard，2023 R1；本项目 `toltool/tde_builder.py` 已封装）

流程：`Initialize()` → 设开关/数值/单位枚举 → 设 `StartAtSurface`/`StopAtSurface` → `OK()`（落 TDE）。向导**只有一个全局面范围**，无逐类范围/跳过面；玻璃/空气/光阑由 Zemax 自动识别。

关键坑点：
- **pythonnet 下单位枚举必须传枚举成员对象，不能传 int。** 命名空间 `ZOSAPI.Wizards`：`DefaultAndFringes`（`.Default/.Fringes/.Percent`）、`DefaultAndDegrees`（`.Default/.Degrees`）。
- 每类公差是「开关属性 + 数值属性」成对；数值属性名随单位变化（见清单）。
- **自写逐面展开与向导在「成对面分组、表面/元件公差区分、玻璃面识别、阿贝换算」四处不等价**，必须用向导，勿手写展开。

精简属性清单（开关 → 数值属性 / 单位）：
| 类别 | 开关 | 数值属性 | 单位处理 |
|---|---|---|---|
| 半径 | `IsSurfaceRadiusUsed` | `SurfaceRadiusFringes`/`SurfaceRadius`/`SurfaceRadiusPercent` | `SurfaceRadiusUnitType`=Fringes/Default/Percent |
| 厚度 | `IsSurfaceThicknessUsed` | `SurfaceThickness` | mm |
| 面偏心X/Y | `IsSurfaceDecenterX/YUsed` | `SurfaceDecenterX/Y` | mm |
| 面倾斜X/Y | `IsSurfaceTiltX/YUsed` | `SurfaceTiltX/YDegrees`/`SurfaceTiltX/Y` | `SurfaceTiltX/YUnitType`=Degrees/Default |
| 元件偏心X/Y | `IsElementDecenterX/YUsed` | `ElementDecenterX/Y` | mm |
| 元件倾斜X/Y | `IsElementTiltX/YUsed` | `ElementTiltXDegrees`/`ElementTiltYDegrees` | 度 |
| 面不规则 | `IsSurfaceSandAIrregularityUsed` | `SurfaceSandAIrregularityFringes` | 光圈 |
| Zernike不规则 | `IsSurfaceZernikeIrregularityUsed` | `SurfaceZernikeIrregularityFringes` | 光圈 |
| 折射率 | `IsIndexUsed` | `Index` | — |
| 阿贝% | `IsIndexAbbePercentageUsed` | `IndexAbbePercentage` | — |
其它：`TestWavelength`（um）、`IsFocusCompensationUsed`（bool）。建议先把上述全部开关置 False，再按需打开。

### 逐行明细与 COMP 补偿器（顺序铁律）

- 直接写 TDE 行：`r=tde.AddOperand()` → `r.ChangeType(ToleranceOperandType.XXX)` → `Param1`/`Param2`/`Min`/`Max`/`Comment`。成对面操作数（`TTHI/TEDX/TEDY/TETX/TETY`）才用 `Param2`，其余置 0。
- **COMP 后焦补偿器必须在「向导 + 明细」之后最后追加，否则被向导清掉。** 语义：`Param1`=补偿面号、`Param2`=0（对该面厚度做补偿）。`Min/Max` 留 0 表示由 Zemax 自由调整。
...
- `.TSC` 放在 `<DataDir>\Tolerance\`，Zemax 列入 `ListOfCriteriaScripts`，`CriterionScript` 取其**整数索引**（Python 端列表项是文件名字符串如 `'05304_tol.TSC'`，取索引）。**`<DataDir>` 定位实测：TDE 无 `ToleranceDirectory` 属性；用 `MFE.MeritFunctionDirectory` 的父目录即数据根目录，再拼 `\Tolerance`。** TSC 用 **UTF-16** 写盘。
...
**读某 MF 行的名义值（实测）**：先调 `MFE.CalculateMeritFunction()`（或 `CalculateMeritFunctionAndUpdateOperands()`）刷新，再读该行 `GetOperandCell(MeritColumn.Value).Value`（列名随版本可能是 `Value`/`CurrentValue`，逐个试）。不刷新会读到过期值。
